...
recursive **bisection** metrical tree, and post-order validation via
**`mcore_py.checker`** (through :func:`mcore_1.check_tree.check_constituent` and
the stable :func:`mcore_1.check_tree.check_tree` weight-stream API).
...
| `tree.py` | `build_binary_metrical_tree`, `build_post_deletion_frozen_tree`, `build_frozen_after_deletion_trits`, `descendant_orig_indices`, `orig_span`. |
| `check_tree.py` | Stable ``NodeResult`` / ``check_tree(weights)`` / ``check_deletion``; :func:`check_constituent` delegates to ``mcore_py.checker.check_tree``. |
...
- `tests/test_cascade.py`, `tests/test_associativity.py`, `tests/test_node_api.py`.
